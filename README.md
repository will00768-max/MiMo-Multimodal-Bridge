...
| 变量 | 默认值 | 说明 |
|------|--------|------|
| `MIMOCODE_SERVER_URL` | `http://localhost:3000` | MiMo Code 服务地址 |
| `MIMOCODE_REQUEST_TIMEOUT_MS` | `120000` | 单次请求超时时间（毫秒），必须为正数 |

工具执行失败时会抛出带有具体原因的错误（文件不存在、读取失败、连接失败、请求超时、HTTP 状态码与响应内容、非 JSON 响应等），而不是返回一段看起来正常的文本。
